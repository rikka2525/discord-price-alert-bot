...
**実際のDiscordサーバーで主要機能と再起動後の設定保持を確認済み。自動テスト5件が成功しています。** 通知・設定管理を扱う自主制作サンプルで、売買機能はありません。
...
本作品はAI支援を活用した個人の自主制作です。受託案件の納品実績ではありません。実装・動作確認の範囲と、未実装の機能を以下に区別して記載しています。
